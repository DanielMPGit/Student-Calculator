...
public class Programa_Clase {
    ...
    static void menuPrincipal() throws Exception {
        String[] opciones = {
            ...
            "  Cambiar ubicacion base de datos",
            "  Salir"
        };
        while (true) {
    ...
    public static void ubicacionBDD() throws Exception {
        String carpeta = pedirInputRuta("Carpeta destino (ejemplo: C:\\Users\\nombre\\carpeta): ");
        if (carpeta == null) return;

        if (!carpeta.endsWith("\\") && !carpeta.endsWith("/")) carpeta += "\\";
        String nombre = carpeta + "colegio.db";
        ...
        boolean ok = DatabaseManager.setDbPath(nombre);
        if (ok) {
            mostrarMensaje(FG_GREEN + "Base de datos movida a:\n  " + nombre + RESET);
        } else {
            mostrarMensaje(FG_RED + "ERROR al mover la base de datos." + RESET);
        }
    }
    ...
    static String pedirInputRuta(String prompt) throws Exception {
        clearScreen();
        cabecera("ENTRADA DE DATOS");
        print(FG_CYAN + "  " + prompt + RESET);
        org.jline.reader.LineReader lr = org.jline.reader.LineReaderBuilder.builder()
